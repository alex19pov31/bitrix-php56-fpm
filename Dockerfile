...
RUN apk add tzdata && \
	...
	apk add openssh curl wget git msmtp libpng-dev freetype libjpeg-turbo-dev libmcrypt-dev freetype-dev libcurl curl-dev libxml2-dev autoconf g++ make && \
	...
	docker-php-ext-install soap mbstring iconv mcrypt mysqli opcache xml zip gd pdo pdo_mysql && \
